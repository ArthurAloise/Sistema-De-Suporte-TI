    ...
    <div class="container-fluid py-4">
        ...
        <div class="d-flex flex-wrap align-items-center justify-content-between mb-4">
            <div class="d-flex align-items-center gap-3">
                ...
                {{-- Título e Descrição --}}
                <div>
                    <h1 class="fw-bolder text-primary mb-0">Categorias de Chamado</h1>
                    <p class="text-muted fs-6 mb-0">Gerencie as categorias principais dos chamados.</p>
                </div>
            </div>
            <a href="{{ route('categories.create') }}" class="btn btn-primary fw-bold shadow-sm mt-2 mt-md-0">
        ...
        <div class="card shadow-lg border-0 rounded-4">
            ...
            <div class="card-header bg-white border-bottom-0 pt-3 pb-0">
                <form method="GET" class="row g-2 align-items-center mb-3" action="{{ route('categories.index') }}">
                    ...
                    <div class="col-auto">
                        ...
                        @if($search)
                            <a href="{{ route('categories.index') }}" class="btn btn-outline-secondary btn-sm ms-1" title="Limpar Busca">
                ...
                <table class="table table-hover table-striped align-middle mb-0">
                    ...
                    @forelse($categories as $category)
                        <tr>
                            ...
                            <td class="fw-semibold">{{ $category->nome }}</td>
                            <td>
                                @if($category->default_priority)
                                    <span class="badge bg-secondary text-capitalize">{{ $category->default_priority }}</span>
                                @else
...
<td class="text-center">
    ...
    @endif
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="text-center text-muted py-4"><i class="fas fa-info-circle me-2"></i>Nenhuma categoria encontrada.</td>
                        </tr>
                    @endforelse
                    </tbody>
                </table>
            ...
            @if($categories->hasPages())
                <div class="card-footer bg-white border-0 py-3">
                    <div class="d-flex justify-content-center">
                         {{ $categories->appends(request()->except('page'))->links('pagination::bootstrap-4') }} {{-- Mantém filtros na paginação --}}
                    </div>
                </div>
            @endif
        </div>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content shadow-lg rounded-4 border-0">
                ...
                <div class="modal-body text-center py-4">
                    <p class="fs-5 mb-1">Tem certeza que deseja excluir a categoria <strong id="categoryname-to-delete"></strong>?</p>
                    <p class="text-danger small fw-bold">Tipos associados também podem ser afetados. Esta ação não pode ser desfeita.</p>
                </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const categoryNameToDeleteSpan = document.getElementById('categoryname-to-delete');
        let deleteUrl = '';
        ...
        confirmDeleteModalElement.addEventListener('hidden.bs.modal', function () {
            deleteUrl = '';
            categoryNameToDeleteSpan.textContent = '';
